...
std::vector<uint8_t> WinKeyIF::CaptureDevelopmentState() const
{
	...
	state[0] = 'W'; state[1] = 1;
	size_t offset = 2;
	memcpy(state.data() + offset, keyport, sizeof(keyport)); offset += sizeof(keyport);
	memcpy(state.data() + offset, keyboard, sizeof(keyboard)); offset += sizeof(keyboard);
	memcpy(state.data() + offset, keystate, sizeof(keystate));
	return state;
}
...
void WinKeyIF::RestoreDevelopmentState(const std::vector<uint8_t>& state)
{
	...
	size_t offset = 2;
	memcpy(keyport, state.data() + offset, sizeof(keyport)); offset += sizeof(keyport);
	memcpy(keyboard, state.data() + offset, sizeof(keyboard)); offset += sizeof(keyboard);
	memcpy(keystate, state.data() + offset, sizeof(keystate));
}
