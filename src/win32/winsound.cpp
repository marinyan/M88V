...
bool WinSound::ChangeRate(uint rate, uint buflen, bool waveout)
{
	if (currentrate != rate || currentbuflen != buflen || wodrv != waveout)
	{
		if (IsDumping())
		{
			winstatusdisplay.Show(70, 3000, "wav 書き出し時の音設定の変更はできません");
			return false;
		}
		...
		if (bufsize > 0)
		{
			for (int i=0; i<1; i++)
			{
				...
				else if (useds2)
				{
					...
					winstatusdisplay.Show(200, 8000, "sounddrv: using Notify driven driver");
				}
				else
				{
					driver = new DriverDS;
					winstatusdisplay.Show(200, 8000, "sounddrv: using timer driven driver");
				}
				...
				if (!driver || !driver->Init(&dumper, hwnd, samprate, 2, buflen))
				{
					...
					{
						useds2 = false;
						winstatusdisplay.Show(100, 3000, "IDirectSoundNotify は使用できないようです");
						i = -1;
					}
				}
			}
			...
			{
				SetRate(rate, 0);
				winstatusdisplay.Show(70, 3000, "オーディオデバイスを使用できません");
			}
		}
	}
	return true;
...
bool SoundDumpPipe::DumpStart(char* filename)
{
	...
	dumpstate_ = STANDBY;
	dumpedsample_ = 0;
	winstatusdisplay.Show(100, 0, "録音待機中～");
	return true;
}
