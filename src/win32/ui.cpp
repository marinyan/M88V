...
bool WinUI::InitM88(const char* cmdline)
{
	...
	//	設定読み込み
	LOG1("%d\tLoadConfig\n", timeGetTime());
	config = PC8801::Config{};
	PC8801::LoadConfig(&config, m88ini, true);
	const char* mode = std::getenv("M88V_BASIC_MODE");
	if (mode && *mode) {
		if (!M88V::ParseBasicMode(mode, &config.basicmode)) {
			MessageBox(hwnd, "Invalid M88V_BASIC_MODE", "M88V", MB_OK | MB_ICONERROR);
			return false;
		}
	}
	...
	if (!keyif.Init(hwnd))
		return false;
	LOG1("%d\tcore\n", timeGetTime());
	if (!core.Init(this, hwnd, &draw, diskmgr, &keyif, &winconfig, tapemgr, config))
		return false;
	...
	LOG1("%d\treset\n", timeGetTime());
	core.Reset();
	if (const char* bin = M88V::EnvironmentValue("M88V_LOAD_BIN", "M88M_LOAD_BIN")) {
		uint16_t address = 0xc000;
		const char* specified = M88V::EnvironmentValue("M88V_LOAD_ADDRESS", "M88M_LOAD_ADDRESS");
		if (specified && !M88V::ParseLoadAddress(specified, &address)) {
			MessageBoxA(hwnd, "Invalid M88V_LOAD_ADDRESS", "M88V", MB_OK | MB_ICONERROR);
			return false;
		}
		std::string message;
		if (!core.LoadBinary(bin, address, &message)) {
			MessageBoxA(hwnd, message.c_str(), "M88V BIN error", MB_OK | MB_ICONERROR);
			return false;
		}
	}
	core.Wait(false);
...
LRESULT WinUI::WmClose(HWND hwnd, WPARAM wparam, LPARAM lparam)
{
	...
	{
		SetGUIFlag(true);
		int res = MessageBox(hwnd, "M88V を終了します", "M88V", MB_ICONEXCLAMATION | MB_OKCANCEL | MB_DEFBUTTON2);
		SetGUIFlag(false);
		if (res != IDOK)
			return 0;
	}
...
LRESULT WinUI::WmTimer(HWND hwnd, WPARAM wparam, LPARAM lparam)
{
	...
	if (wparam == timerid)
	{
		...
		if (report)
		{
			if (active)
			{
				...
				wsprintf(buf, "M88V - %d fps.  %d.%.2d MHz",
					fcount, freq / 100, freq % 100);
				SetWindowText(hwnd, buf);
			}
			else
				SetWindowText(hwnd, "M88V");
		}
	...
	if (wparam == winstatusdisplay.GetTimerID())
	{
		winstatusdisplay.Update();
		return 0;
	}
	return 0;
}
...
void WinUI::ReportError()
{
	...
	if (errtext)
	{
		::MessageBox(hwnd, errtext, "M88V", MB_ICONERROR | MB_OK);
	}
}
...
void WinUI::Reset()
{
	...
	{
		SetGUIFlag(true);
		int res = MessageBox(hwnd, "リセットしますか？", "M88V", MB_ICONQUESTION | MB_OKCANCEL | MB_DEFBUTTON2);
		SetGUIFlag(false);
		if (res != IDOK)
			return;
	}
...
void WinUI::ResizeWindow(uint width, uint height)
{
	...
	rect.top  = 0;  rect.bottom = height + winstatusdisplay.GetHeight();
...
{
	if ((UINT) wparam == 1)
		winstatusdisplay.DrawItem((DRAWITEMSTRUCT*)lparam);
	return TRUE;
}
...
LRESULT WinUI::WmDropFiles(HWND hwnd, WPARAM wparam, LPARAM lparam)
{
	...
	if (nfiles != 1)
	{
		winstatusdisplay.Show(50, 3000, "ドロップできるのはファイル１つだけです.");
		DragFinish(hdrop);
		return 0;
	}
	...
	if (strcmpi(ext, ".d88") == 0) {
		...
		winstatusdisplay.Show(50, 3000, "非対応のファイルです.");
	}
...
void WinUI::CaptureScreen()
{
	...
	if (bmpsize)
	{
		...
		if (config.flag2 & Config::genscrnshotname)
		{
			...
			save = true;
			winstatusdisplay.Show(80, 1500, "画面イメージを %s に保存しました", filename);
		}
		else
		{
			filename[0] = 0;
...
//	winstatusdisplay.Show(0, 0, "%d", LOWORD(lp));
...
void WinUI::SaveSnapshot(int n)
{
	...
	if (core.SaveShapshot(name))
		winstatusdisplay.Show(80, 3000, "%s に保存しました", name);
	else
		winstatusdisplay.Show(80, 3000, "%s に保存できません", name);
	currentsnapshot = n;
	snapshotchanged = true;
}
...
void WinUI::LoadSnapshot(int n)
{
	...
	if (r)
		winstatusdisplay.Show(80, 2500, "%s から復元しました", name);
	else
		winstatusdisplay.Show(80, 2500, "%s から復元できません", name);
	for (uint i=0; i<2; i++)
		CreateDiskMenu(i);
	currentsnapshot = n;
	snapshotchanged = true;
