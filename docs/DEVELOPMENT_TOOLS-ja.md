...
CRTC／600Hz精度改善後のチェックポイント形式は2です。端数位相を保存するため、
旧形式1の状態ファイルとそれを含む入力リプレイは再作成してください。
詳細は [タイミング改善](CRTC_TIMING-ja.md) を参照。
...
フェッチキャッシュ・ADPCMの再生位置とデコーダ状態・キーマトリクス・フレーム位置を含めます。headlessのカレンダーは
...
- 精密チェックポイントは形式5です（YM2203/YM2608のFM内部状態と音源ごとの分周設定を追加）。形式1〜4の状態ファイルと、それを含むリプレイは再作成してください。
